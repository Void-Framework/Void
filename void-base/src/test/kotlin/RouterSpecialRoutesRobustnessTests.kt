...
class RouterSpecialRoutesRobustnessTests {
    ...
    fun throwing_high_priority_allows_lower_priority_to_handle() {
        ...
        val low: (RequestDTO, Map<String, String>, io.voidx.ClientHandler) -> ResponseDTO? = { _, _, _ -> ok("low") }
    ...
    fun unregister_during_dispatch_is_safe_and_effective_next_request() {
        val r = router { }
        var count = 0
        lateinit var self: (RequestDTO, Map<String, String>, io.voidx.ClientHandler) -> ResponseDTO?
        self = { _, _, _ ->
            count += 1
        ...
        val other = { _: RequestDTO, _: Map<String, String>, _: io.voidx.ClientHandler -> ok("ok") }
