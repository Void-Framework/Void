...
class RouterRouteValidationTests {
    ...
    fun addRoute_throws_when_target_missing_leading_slash() {
        ...
        val bad: Page = route("no-slash") { GET { _, _ -> ok("x", mutableMapOf("Content-Type" to "text/plain")) } }
    ...
    fun addRoute_throws_when_duplicate_target_used() {
        val r = Router()
        val p1: Page = route("/dup") { GET { _, _ -> ok("1", mutableMapOf("Content-Type" to "text/plain")) } }
        val p2: Page = route("/dup") { GET { _, _ -> ok("2", mutableMapOf("Content-Type" to "text/plain")) } }
        r.addRoute(p1)
